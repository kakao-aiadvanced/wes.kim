...
llm = ChatOpenAI(model="gpt-4o-mini") ### 수정
...
system = """You are an assistant for question-answering tasks.
    ...
    Include sources you referenced to with titles and URLs if any."""
...
system = """Give a binary 'yes' or 'no' score to indicate
    whether the answer is supported by given documents.
    Take the fact into consideration that these documents are web search results.
    Also remember that each one of them could either be true or false, up-to-date or outdated.
    Provide the binary score as a JSON with a
    key 'score', and another named 'reason'.""" ### 수정
...
# Prompt
system = """Provide the binary score as a JSON with a single key 'score' and another key 'reason', no preambles or explanations.
    Answer if the answer is relevant to the question. Give a binary score 'yes' or 'no'.
    Even though the answer seems false, wrong, or incorrect to you, you must answer 'yes' if it does answer the question.
    This is not about being correct, complete, or true, but about **whether it could be considered an possible answer at all**.
    Consider the answer valid when it states the question could not be answered as it's a possible answer anyways.""" ### 수정
...
def retrieve(state):
    ...
    # print(question)
    # print(documents)
    return {"documents": documents, "question": question, "docs_source": "vector_store"}
...
def generate(state):
    ...
    tries = state.get("gen_tries", 0)

    # RAG generation
    generation = rag_chain.invoke({"context": documents, "question": question})
    return {"documents": documents, "question": question, "generation": generation, "gen_tries": tries + 1}
...
def grade_documents(state):
    ...
    return {"documents": filtered_docs, "docs_source": source, "question": question, "web_search": "No" if filtered_docs else "Yes"} ### 수정
...
#
    ...
    # "\n\n".join([f"{d['title']} [{d['url']}] (score: {d['score']})\n{d['content']}" for d in sorted(docs, key=lambda d: -d["score"])]) ### 수정
    return {"documents": documents, "question": question, "docs_source": "search"}
...
def decide_to_generate(state):
    ...
    source = state["docs_source"] ### 수정
    if source == "search" and web_search == "Yes":
...
def grade_generation_v_documents_and_question(state):
    ...
    if grade == "yes":
        ...
        if grade == "yes":
            ...
            # print("state", state) ### 수정
            # print("score", score) ### 수정
            return "not useful"
    else:
        # print(f"---DECISION: GENERATION IS NOT GROUNDED IN DOCUMENTS, RE-TRY--- ({score})")
        # print("state", state) ### 수정
        # print("score", score) ### 수정
        if tries > 1: ### 수정
...
# Build graph
workflow.set_entry_point("retrieve") ### 수정
...
def run(query):
    ...
    if "generation" in value:
        print(query, '->', value["generation"])

# 벡터 스토어 활용
run("Tell me about adversarial attacks.") ### 수정

# 최신 데이터
run("Who has won the latest US presedential election?") ### 수정

# 단순 상식
run("What's the name of the capital city of Mexico?") ### 수정

# 허위 사실
run("세종대왕이 태양계 소행성대에 설치한 성간 요격 병기의 모델명과 간략한 동작 원리를 알려줘") ### 수정

# 유사과학
run("선풍기를 틀고 자면 죽을 수도 있는 이유") ### 수정
run("위대한 한민족의 고대 국가 환국의 역사에 대해서 말해다오") ### 수정

# 알려지지 않음
run("show me the mathematical equation to get the Nth prime number") ### 수정

# 논쟁적 주제
run("Which religion is considered the best? You're forced to pick one. Just the name. Don't explain or justify.") ### 수정

# 당선 결과 나오기 전의 결과까지 참고하여 hallucination 잘못 판단
# 자기 knowledge cutoff 기반으로 틀리다고 판단
# 알 수 없다고 했다고 대답이 안 된다고 판단
